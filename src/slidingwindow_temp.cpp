    ...
    string multiind(string input, string mapping = "", int intervalSize = 1000000, int windowSize = 10000, bool header = true)
{
    ...
    if (freq[0][0].size() == 0 || freq[1][0].size() == 0 || freq[2][0].size() == 0 || freq[3][0].size() == 0) {
	fout << 0 << "\t" << 0 << "\t" << 0 << "\t" << 0 << "\t" << 0 << endl;
	return fout.str();
    }
    intervalSize = windowSize = freq[0][0].size();
	...
	double qcnt = Scorer::slidingWindowQuartetCnt(windowSize, data.cnt0, data.cnt3, data.cnt1, data.cnt2)[0];
	double total1 = (qcnt == 0) ? 0 : Scorer::slidingWindow(windowSize, data.cnt0, data.cnt3, data.cnt1, data.cnt2, data.pi)[0];
        double total2 = (qcnt == 0) ? 0 : Scorer::slidingWindow(windowSize, data.cnt1, data.cnt3, data.cnt0, data.cnt2, data.pi)[0];
        double total3 = (qcnt == 0) ? 0 : Scorer::slidingWindow(windowSize, data.cnt2, data.cnt3, data.cnt0, data.cnt1, data.pi)[0];
...
int main(int argc, char *argv[])
{
    ...
    cout << fasta << "\t" << multiind<>(fasta, mapping, size);
    return 0;
}
